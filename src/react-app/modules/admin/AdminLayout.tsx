...
export function AdminLayout() {
  ...
  const businessImageKey = data?.business.coverImageKey ?? data?.business.faviconKey
  const businessImageIsCover = Boolean(data?.business.coverImageKey)
  ...
  return (
    <div className="min-h-[100dvh] bg-admin-background" style={brandTheme}>
      ...
      <aside id="admin-sidebar" className={cn('fixed inset-y-0 left-0 z-[70] flex w-[min(18rem,86vw)] -translate-x-[102%] flex-col gap-6 bg-[#292520] p-[1.1rem] text-[#eeeae4] transition-transform duration-200 ease-in-out motion-reduce:transition-none lg:w-[16.5rem] lg:translate-x-0', open && 'translate-x-0')} aria-hidden={!desktop && !open} inert={!desktop && !open ? true : undefined}>
        <div className="flex items-center justify-between gap-[.6rem]"><div className="flex items-center gap-3 p-[.35rem]">{businessImageKey ? <img className={cn('h-10 w-10 shrink-0 rounded-[.75rem] bg-[var(--color-brand)]', businessImageIsCover ? 'object-cover' : 'object-contain p-1')} src={`/media/${businessImageKey}`} alt={`${businessImageIsCover ? 'Capa' : 'Logo'} de ${businessName}`} /> : <span className="grid h-10 w-10 shrink-0 place-items-center rounded-[.75rem] bg-[var(--color-brand)] font-[900] text-[var(--color-brand-text)]">{businessInitial}</span>}<div className="grid min-w-0"><strong className="overflow-hidden text-ellipsis whitespace-nowrap">{businessName}</strong><small className="text-[#aaa39a]">Administração</small></div></div><button ref={closeButtonRef} className={`inline-flex min-h-11 cursor-pointer items-center gap-[.35rem] rounded-[.55rem] border border-[#5b5249] bg-transparent px-[.65rem] py-[.55rem] font-[700] text-white lg:hidden ${focusRing}`} type="button" onClick={closeMenu}><X className="w-4" /> <span>Fechar</span></button></div>
