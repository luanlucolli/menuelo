  ...
  return (
    <div className="min-h-[100dvh] bg-admin-background" style={brandTheme}>
      <header className="sticky top-0 z-[60] flex h-16 items-center gap-2 border-b border-border bg-surface-strong px-4 lg:hidden">
        <button
          className={`grid h-11 w-11 place-items-center rounded-[.6rem] bg-transparent ${focusRing}`}
          ref={menuButtonRef}
          type="button"
      ...
      {open && <button type="button" className={`fixed inset-0 z-[69] border-0 bg-[rgb(24_20_17_/_55%)] ${focusRing}`} aria-label="Fechar menu" onClick={closeMenu} />}
      <aside id="admin-sidebar" className={cn('fixed inset-y-0 left-0 z-[70] flex w-[min(18rem,86vw)] -translate-x-[102%] flex-col gap-6 bg-[#292520] p-[1.1rem] text-[#eeeae4] transition-transform duration-200 ease-in-out lg:w-[16.5rem] lg:translate-x-0', open && 'translate-x-0')} aria-hidden={!desktop && !open} inert={!desktop && !open ? true : undefined}>
        <div className="flex items-center justify-between gap-[.6rem]"><div className="flex items-center gap-3 p-[.35rem]"><span className="grid h-10 w-10 shrink-0 place-items-center rounded-[.75rem] bg-[var(--color-brand)] font-[900] text-[var(--color-brand-text)]">{businessInitial}</span><div className="grid min-w-0"><strong className="overflow-hidden text-ellipsis whitespace-nowrap">{businessName}</strong><small className="text-[#aaa39a]">Administração</small></div></div><button ref={closeButtonRef} className={`inline-flex min-h-11 items-center gap-[.35rem] rounded-[.55rem] border border-[#5b5249] bg-transparent px-[.65rem] py-[.55rem] font-[700] text-white lg:hidden ${focusRing}`} type="button" onClick={closeMenu}><X className="w-4" /> <span>Fechar</span></button></div>
