...
function SortableCategory({ category, index, total, organizing, onEdit, onDelete, onMove }: { category: Category; index: number; total: number; organizing: boolean; onEdit: () => void; onDelete: () => void; onMove: (direction: -1 | 1) => void }) {
  ...
  return <article ref={setNodeRef} style={{ transform: CSS.Transform.toString(transform), transition }} className={cn('flex min-w-0 items-center gap-2 border-b border-border bg-surface-strong px-[.35rem] py-[.65rem] last:border-b-0 max-[649px]:grid max-[649px]:grid-cols-[minmax(0,1fr)_auto] max-[649px]:gap-[.55rem]', organizing && 'max-[649px]:grid-cols-[44px_minmax(0,1fr)]', isDragging && 'z-[5] rounded-[.6rem] border border-[var(--color-brand)] shadow-menu-lg')}>
    {organizing && <button className={`grid h-11 w-11 shrink-0 cursor-grab place-items-center rounded-[.5rem] bg-transparent [touch-action:none] ${focusRing}`} type="button" aria-label={`Arrastar ${category.name}`} {...attributes} {...listeners}><GripVertical className="w-[1.15rem] text-muted" /></button>}
    ...
    <div className="flex flex-wrap justify-end gap-[.2rem] [&_svg]:h-4 [&_svg]:w-4 max-[649px]:col-span-full max-[649px]:grid max-[649px]:grid-cols-2">{organizing ? <><button className={`inline-flex min-h-11 min-w-11 cursor-pointer items-center justify-center gap-[.3rem] rounded-[.45rem] bg-transparent px-[.55rem] py-[.45rem] text-[.76rem] font-[700] disabled:cursor-not-allowed disabled:opacity-50 max-[649px]:bg-[#f5f2ed] ${focusRing}`} type="button" disabled={index === 0} onClick={() => onMove(-1)}><ArrowUp /><span>Subir</span></button><button className={`inline-flex min-h-11 min-w-11 cursor-pointer items-center justify-center gap-[.3rem] rounded-[.45rem] bg-transparent px-[.55rem] py-[.45rem] text-[.76rem] font-[700] disabled:cursor-not-allowed disabled:opacity-50 max-[649px]:bg-[#f5f2ed] ${focusRing}`} type="button" disabled={index === total - 1} onClick={() => onMove(1)}><ArrowDown /><span>Descer</span></button></> : <><button className={`inline-flex min-h-11 min-w-11 cursor-pointer items-center justify-center gap-[.3rem] rounded-[.45rem] bg-transparent px-[.55rem] py-[.45rem] text-[.76rem] font-[700] ${focusRing}`} type="button" onClick={onEdit}><Pencil /><span>Editar</span></button><button className={`inline-flex min-h-11 min-w-11 cursor-pointer items-center justify-center gap-[.3rem] rounded-[.45rem] bg-transparent px-[.55rem] py-[.45rem] text-[.76rem] font-[700] text-danger max-[649px]:bg-[#f5f2ed] ${focusRing}`} type="button" onClick={onDelete}><Trash2 /><span>Excluir</span></button></>}</div>
  </article>
}
...
export function CategoriesPage() {
  ...
  return <div className="mx-auto w-full max-w-[72rem]">
    ...
    {editing && <AdminDialog onClose={requestFormClose}><section className="w-full max-h-[94dvh] overflow-y-auto rounded-[1.2rem_1.2rem_0_0] bg-surface-strong p-4 shadow-menu-lg min-[650px]:m-auto min-[650px]:max-h-[calc(100dvh_-_3rem)] min-[650px]:w-[min(100%,42rem)] min-[650px]:rounded-[1rem]" aria-labelledby="category-form-title"><div className="mb-4 flex items-center justify-between gap-4"><h2 className="m-0" id="category-form-title">{editing === 'new' ? 'Nova categoria' : 'Editar categoria'}</h2><button className={`grid h-11 w-11 cursor-pointer place-items-center rounded-full bg-[#efebe5] text-[1.5rem] ${focusRing}`} type="button" aria-label="Fechar" onClick={requestFormClose}><X /></button></div><form className="grid gap-[.9rem]" noValidate onSubmit={form.handleSubmit((input) => save.mutate(input))}>
