    ...
    <section className="grid min-w-0 gap-3 rounded-[.75rem] border border-border bg-[#faf8f4] p-[.85rem]">
      <div className="flex items-start justify-between gap-[.65rem]">
        ...
        <div className="flex flex-wrap items-center justify-end gap-[.2rem]">
          ...
          <button type="button" className={`inline-flex min-h-9 items-center gap-[.35rem] border-0 bg-transparent px-[.35rem] text-[.75rem] font-[750] text-danger ${focusRing}`} onClick={() => onRemove(index)}><Trash2 className="w-4" /> Remover grupo</button>
        </div>
      </div>
      ...
      <div className="grid gap-3 border-t border-border pt-3">
        {options.fields.map((option, optionIndex) => {
          ...
          return <section className="grid gap-[.65rem] rounded-[.65rem] border border-border bg-white p-3" key={option.id}>
            ...
            <label className={fieldLabel}>Descrição <span className="font-[500] text-muted">(opcional)</span><textarea className={`${textInput} resize-y`} rows={2} maxLength={500} placeholder="Ex.: pão brioche e hambúrguer artesanal" {...form.register(`${optionName}.description`, { setValueAs: (value) => value || null })} /></label>
