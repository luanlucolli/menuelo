...
export function ProductForm({ product, categories, initialCategoryId, onClose, onSaved, onCategoryCreated }: { product: Product | null; categories: Category[]; initialCategoryId?: string; onClose: () => void; onSaved: (message: string) => Promise<void>; onCategoryCreated?: () => Promise<void> }) {
  ...
  return <AdminDialog onClose={requestClose}><section className="w-full max-h-[94dvh] overflow-y-auto rounded-[1.2rem_1.2rem_0_0] bg-surface-strong p-4 shadow-menu-lg min-[650px]:m-auto min-[650px]:max-h-[calc(100dvh_-_3rem)] min-[650px]:w-[min(100%,68rem)] min-[650px]:rounded-[1rem]" aria-labelledby="product-form-title">
