...
export function ProductForm({ product, categories, initialCategoryId, onClose, onSaved, onCategoryCreated }: { product: Product | null; categories: Category[]; initialCategoryId?: string; onClose: () => void; onSaved: (message: string) => Promise<void>; onCategoryCreated?: () => Promise<void> }) {
  ...
  return <AdminDialog onClose={requestClose}><section className="admin-form-dialog product-form-dialog" aria-labelledby="product-form-title">
    <div className="form-dialog-heading"><div><h2 id="product-form-title">{product ? 'Editar produto' : 'Novo produto'}</h2><p>{product ? 'Altere os dados que identificam o produto.' : 'Informe nome, categoria e preço para começar.'}</p></div><button type="button" aria-label="Fechar" onClick={requestClose}><X /></button></div>
    <form noValidate onSubmit={form.handleSubmit((input) => save.mutate(input))}>
      ...
      <section className="essential-fields" aria-labelledby="essential-title">
        ...
        {!product && <Controller control={form.control} name="variants.0.priceCents" render={({ field }) => <MoneyInput id="product-main-price" label="Preço" value={field.value > 0 ? field.value : null} onChange={(value) => field.onChange(value ?? 0)} error={firstVariantError?.priceCents?.message} />} />}
      </section>
      ...
      {product && <aside className="product-form-scope" aria-label="Onde alterar preço e disponibilidade"><strong>Preços, promoções, tamanhos e disponibilidade</strong><span>Essas alterações ficam nos botões da lista de produtos.</span></aside>}
