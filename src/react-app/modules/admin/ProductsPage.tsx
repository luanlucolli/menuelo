...
function SortableProduct({ product, categoryName, index, total, orderingEnabled, availabilityBusy, onMove, onEdit, onToggleAvailability, onDuplicate, onDelete }: { product: Product; categoryName: string; index: number; total: number; orderingEnabled: boolean; availabilityBusy: boolean; onMove: (direction: -1 | 1) => void; onEdit: () => void; onToggleAvailability: () => void; onDuplicate: () => void; onDelete: () => void }) {
  ...
  return <article ref={setNodeRef} style={{ transform: CSS.Transform.toString(transform), transition }} className={cn('flex min-w-0 items-center gap-2 border-b border-border bg-surface-strong px-[.35rem] py-[.65rem] last:border-b-0 max-[649px]:grid max-[649px]:grid-cols-[auto_minmax(0,1fr)] max-[649px]:gap-[.6rem] max-[649px]:px-2 max-[649px]:py-3', orderingEnabled && 'max-[649px]:grid-cols-[44px_auto_minmax(0,1fr)]', !product.isAvailable && 'bg-[#fff8f7]', isDragging && 'z-[5] rounded-[.6rem] border border-[var(--color-brand)] shadow-menu-lg')}>
    {orderingEnabled && <button className={`grid h-11 w-11 shrink-0 cursor-grab place-items-center rounded-[.5rem] bg-transparent [touch-action:none] ${focusRing}`} type="button" aria-label={`Arrastar ${product.name}`} {...attributes} {...listeners}><GripVertical className="w-[1.15rem] text-muted" /></button>}
    ...
    <div className="flex flex-wrap justify-end gap-[.2rem] [&_svg]:h-4 [&_svg]:w-4 max-[649px]:col-span-full max-[649px]:grid max-[649px]:grid-cols-2 max-[649px]:border-t max-[649px]:border-border max-[649px]:pt-[.45rem]">
      {orderingEnabled ? <><button className={`inline-flex min-h-11 min-w-11 cursor-pointer items-center justify-center gap-[.3rem] rounded-[.45rem] bg-transparent px-[.55rem] py-[.45rem] text-[.76rem] font-[700] disabled:cursor-not-allowed disabled:opacity-50 max-[649px]:bg-[#f5f2ed] ${focusRing}`} type="button" disabled={index === 0} onClick={() => onMove(-1)}><ArrowUp /><span>Subir</span></button><button className={`inline-flex min-h-11 min-w-11 cursor-pointer items-center justify-center gap-[.3rem] rounded-[.45rem] bg-transparent px-[.55rem] py-[.45rem] text-[.76rem] font-[700] disabled:cursor-not-allowed disabled:opacity-50 max-[649px]:bg-[#f5f2ed] ${focusRing}`} type="button" disabled={index === total - 1} onClick={() => onMove(1)}><ArrowDown /><span>Descer</span></button></> : <>
        <button type="button" className={cn('inline-flex min-h-11 min-w-11 cursor-pointer items-center justify-center gap-[.3rem] rounded-[.45rem] bg-transparent px-[.55rem] py-[.45rem] text-[.76rem] font-[700] text-danger max-[649px]:col-span-full max-[649px]:w-full max-[649px]:bg-[#f5f2ed]', product.isAvailable && 'text-success', focusRing)} aria-pressed={!product.isAvailable} disabled={availabilityBusy} onClick={onToggleAvailability}>{product.isAvailable ? <CircleOff /> : <CircleCheck />}<span>{availabilityBusy ? 'Salvando…' : product.isAvailable ? 'Indisponibilizar' : 'Disponibilizar'}</span></button>
        <button className={`inline-flex min-h-11 min-w-11 cursor-pointer items-center justify-center gap-[.3rem] rounded-[.45rem] bg-transparent px-[.55rem] py-[.45rem] text-[.76rem] font-[700] ${focusRing}`} type="button" onClick={onEdit}><Pencil /><span>Editar dados</span></button>
        <details className="relative"><summary className={`inline-flex min-h-11 min-w-11 cursor-pointer list-none items-center justify-center gap-[.3rem] rounded-[.45rem] px-[.55rem] py-[.45rem] text-[.76rem] font-[700] marker:hidden ${focusRing}`}><Ellipsis /><span>Mais ações</span></summary><div className="absolute right-0 top-[calc(100%+.3rem)] z-[25] grid min-w-40 rounded-[.6rem] border border-border bg-surface-strong p-[.3rem] shadow-menu-lg"><button className={`flex min-h-11 w-full cursor-pointer items-center justify-start gap-[.3rem] rounded-[.45rem] bg-transparent px-[.55rem] py-[.45rem] font-[700] ${focusRing}`} type="button" onClick={(event) => { event.currentTarget.closest('details')?.removeAttribute('open'); onDuplicate() }}><Copy /><span>Duplicar</span></button><button className={`flex min-h-11 w-full cursor-pointer items-center justify-start gap-[.3rem] rounded-[.45rem] bg-transparent px-[.55rem] py-[.45rem] font-[700] text-danger ${focusRing}`} type="button" onClick={(event) => { event.currentTarget.closest('details')?.removeAttribute('open'); onDelete() }}><Trash2 /><span>Excluir</span></button></div></details>
      </>}
    </div>
  </article>
}
