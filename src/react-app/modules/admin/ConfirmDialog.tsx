...
export function ConfirmDialog({ title, description, details, confirmLabel, busy = false, onConfirm, onClose }: { title: string; description: string; details?: ReactNode; confirmLabel: string; busy?: boolean; onConfirm: () => void; onClose: () => void }) {
  ...
  return (
    <AdminDialog onClose={close}>
      <section className="w-[min(100%,31rem)] max-h-[94dvh] overflow-y-auto rounded-[1.2rem_1.2rem_0_0] bg-surface-strong p-4 shadow-menu-lg min-[650px]:m-auto min-[650px]:max-h-[calc(100dvh_-_3rem)] min-[650px]:w-[min(100%,42rem)] min-[650px]:rounded-[1rem]" aria-labelledby="confirm-dialog-title" aria-describedby="confirm-dialog-description">
