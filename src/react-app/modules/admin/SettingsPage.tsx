...
import { ArrowRight, Clock3, CreditCard, Download, ImageIcon, MapPin, Pencil, Plus, Settings2, Store, Trash2 } from 'lucide-react'
...
const settingsFieldsBySection: Partial<Record<SettingsSection, readonly (keyof SettingsInput)[]>> = {
  ...
  advanced: ['slug', 'timezone', 'publicSiteUrl', 'seoTitle', 'seoDescription'],
}
...
export function SettingsPage() {
  ...
  const save = useMutation({ mutationFn: (input: SettingsInput) => api<BusinessSettings>('/admin/api/settings', { method: 'PATCH', body: jsonBody(input) }), onSuccess: async (saved) => { form.reset(settingsFromBusiness(saved)); setFeedback({ kind: 'success', message: 'Informações salvas.' }); await refresh() }, onError: (cause) => setFeedback({ kind: 'error', message: messageFromError(cause) }) })

  const uploadCover = async (file: File) => {
    setImageBusy(true); setImageProgress(null); setFeedback(null)
    try { const blob = await prepareImage(file); setImageProgress(0); await uploadBlob('/admin/api/settings/cover-image', blob, setImageProgress); setFeedback({ kind: 'success', message: form.formState.isDirty ? 'Capa salva automaticamente. A alteração de cor ainda precisa ser salva.' : 'Capa atualizada e salva automaticamente.' }); await refresh() } catch (cause) { setFeedback({ kind: 'error', message: messageFromError(cause) }) } finally { setImageBusy(false); setImageProgress(null) }
  }
  const removeCover = async () => { try { await api('/admin/api/settings/cover-image', { method: 'DELETE' }); setConfirmCoverRemoval(false); setFeedback({ kind: 'success', message: form.formState.isDirty ? 'Capa removida e salva automaticamente. A alteração de cor ainda precisa ser salva.' : 'Capa removida e alteração salva automaticamente.' }); await refresh() } catch (cause) { setConfirmCoverRemoval(false); setFeedback({ kind: 'error', message: messageFromError(cause) }) } }
