...
export function SettingsPage() {
  ...
  const uploadCover = async (file: File) => {
    ...
    try { const blob = await prepareImage(file); setImageProgress(0); await uploadBlob('/admin/api/settings/cover-image', blob, setImageProgress); setFeedback(publicChangeNotice(form.formState.isDirty ? 'Capa salva automaticamente. As alterações de aparência ainda precisam ser salvas.' : 'Capa atualizada e salva automaticamente.')); await refresh() } catch (cause) { setFeedback({ kind: 'error', message: messageFromError(cause) }) } finally { setImageBusy(false); setImageProgress(null) }
  }
  const removeCover = async () => { try { await api('/admin/api/settings/cover-image', { method: 'DELETE' }); setConfirmCoverRemoval(false); setFeedback(publicChangeNotice(form.formState.isDirty ? 'Capa removida e salva automaticamente. As alterações de aparência ainda precisam ser salvas.' : 'Capa removida e alteração salva automaticamente.')); await refresh() } catch (cause) { setConfirmCoverRemoval(false); setFeedback({ kind: 'error', message: messageFromError(cause) }) } }
  ...
  return <div className="mx-auto w-full max-w-[72rem]"><div className="mb-[1.3rem] flex flex-wrap items-end justify-between gap-4"><div><p className="mb-[.2rem] text-[.75rem] font-[850] uppercase tracking-[.1em] text-[var(--color-brand-strong)]">Estabelecimento</p><h1 className="m-0 text-[clamp(1.8rem,7vw,2.5rem)] tracking-[-.035em]">Configurações</h1><span className="mt-[.35rem] block text-muted">Escolha uma seção e altere somente o que precisa.</span></div></div>
    ...
    {formSection && <form className="mt-4 grid gap-4" noValidate onSubmit={(event) => void submitSettings(event)}>
      ...
      {activeSection === 'appearance' && <div className="mt-0 grid gap-4"><section className={settingsCard}><Controller control={form.control} name="theme" render={({ field }) => <MenuThemeSelector value={field.value} onChange={field.onChange} onBlur={field.onBlur} inputRef={field.ref} />} /></section><section className={settingsCard}><div className={cardHeading}><div><h2 className="m-0 text-[1.1rem]">Cor principal do cardápio</h2><p className="mt-1 text-[.88rem] text-muted">Usada nos destaques, botões e detalhes vistos pelos clientes.</p></div></div><Controller control={form.control} name="primaryColor" render={({ field }) => {
