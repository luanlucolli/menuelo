...
export function ProductCustomizationGroups({ product, draft, onChange }: { product: Product; draft: CustomizationDraft; onChange: (draft: CustomizationDraft) => void }) {
  ...
  return <div className="mt-6 grid gap-[22px]">
    {groups.map((group) => {
      ...
      return <fieldset className="m-0 min-w-0 border-0 p-0" key={group.id} aria-describedby={`customization-help-${group.id}`}>
        ...
        <div className="grid min-w-0 gap-2">
          {group.options.map((option) => {
            ...
            if (group.maxSelections === 1) {
              ...
              return <label className={cn('grid min-h-[58px] cursor-pointer grid-cols-[22px_minmax(0,1fr)_auto] items-center gap-[10px] rounded-xl border border-menu-border px-3 py-2 text-[.88rem] font-[680] focus-within:outline-[3px] focus-within:outline-[color-mix(in_srgb,var(--color-brand)_28%,transparent)] focus-within:outline-offset-2', selected && 'border-[var(--color-brand)] bg-[color-mix(in_srgb,var(--color-brand)_7%,#fff)]')} htmlFor={inputId} key={option.id}>
            ...
            return <div className={cn('flex min-h-[70px] min-w-0 items-center justify-between gap-3 rounded-xl border border-menu-border bg-menu-surface px-3 py-2.5 aria-[selected=true]:!border-[var(--color-brand)] aria-[selected=true]:!bg-[color-mix(in_srgb,var(--color-brand)_7%,#fff)]', selected && 'border-[var(--color-brand)] bg-[color-mix(in_srgb,var(--color-brand)_7%,#fff)]')} key={option.id} aria-selected={selected}>
