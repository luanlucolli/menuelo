...
function CartLineItem({
  ...
  return (
    <article className="menu-cart-line border-b border-menu-border py-5 last:border-b-0">
      ...
      {line.note && !editingNote && (
        <div className="menu-cart-note mt-3 rounded-[10px] bg-menu-surface-muted px-[11px] py-2.5">
...
export function CartDialog({
  ...
  return (
    <dialog
      ref={dialogRef}
      className="menu-cart-dialog fixed inset-0 m-auto h-auto max-h-[min(90dvh,820px)] w-[min(calc(100%_-_30px),620px)] max-w-none overflow-visible border-0 bg-transparent p-0 text-menu-text motion-reduce:scroll-auto max-[639px]:inset-auto max-[639px]:right-0 max-[639px]:bottom-0 max-[639px]:left-0 max-[639px]:m-0 max-[639px]:max-h-[calc(100dvh_-_max(8px,env(safe-area-inset-top)))] max-[639px]:w-full max-[639px]:overflow-hidden backdrop:bg-[rgb(24_21_18_/_68%)] backdrop:backdrop-blur-[2px]"
      aria-labelledby="menu-cart-dialog-title"
      onClick={(event) => {
        if (event.target === event.currentTarget) event.currentTarget.close()
      }}
    >
      <section className="menu-cart-sheet flex max-h-[min(90dvh,820px)] w-full flex-col overflow-hidden rounded-[22px] bg-menu-surface shadow-[0_28px_70px_rgb(0_0_0_/_34%)] motion-reduce:scroll-auto max-[639px]:h-[calc(100dvh_-_max(8px,env(safe-area-inset-top)))] max-[639px]:max-h-none max-[639px]:rounded-[22px_22px_0_0]">
        <header className="menu-cart-header flex min-h-[68px] shrink-0 items-center justify-between border-b border-menu-border bg-menu-surface px-[14px] py-3 pl-5 max-[639px]:pl-[18px]">
        ...
        <div className="menu-cart-content min-h-0 flex-1 overflow-y-auto overscroll-contain px-5 [-webkit-overflow-scrolling:touch] motion-reduce:scroll-auto max-[639px]:px-[18px]">
          {!lines.length ? (
        ...
        {lines.length > 0 && (
          <footer className="menu-cart-footer shrink-0 border-t border-menu-border bg-menu-surface px-5 pb-[max(16px,env(safe-area-inset-bottom))] pt-3.5 shadow-[0_-7px_20px_rgb(27_23_19_/_7%)] max-[639px]:px-[18px]">
            ...
            {whatsappUrl ? (
              ...
              <button className={`mt-3 flex min-h-[52px] w-full items-center justify-center rounded-[13px] border-0 bg-menu-control-disabled px-3.5 py-2.5 text-center text-[.9rem] font-[800] text-menu-control-disabled-text disabled:cursor-not-allowed ${publicCartFocusRing}`} type="button" disabled>
                Enviar pedido pelo WhatsApp
              </button>
            )}
