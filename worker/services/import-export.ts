...
export async function applyImport(db: D1Database, bucket: R2Bucket, data: MenuImport): Promise<ImportSummary> {
  ...
  const statements: D1PreparedStatement[] = [
    db.prepare(`UPDATE business_settings SET name=?, slug=?, slogan=?, description=?, whatsapp=?, phone=?, instagram_url=?, facebook_url=?, address=?, address_postal_code=?, address_street=?, address_number=?, address_complement=?, address_neighborhood=?, address_city=?, address_state=?, maps_url=?, timezone=?, special_message=?, theme=?, primary_color=?, cover_image_key=?, favicon_key=?, public_site_url=?, seo_title=?, seo_description=?, updated_at=strftime('%Y-%m-%dT%H:%M:%fZ', 'now') WHERE id=1`).bind(
      data.business.name,
      data.business.slug,
