...
adminRoutes.patch('/settings', async (c) => {
  ...
  await c.env.DB.prepare(`UPDATE business_settings SET name=?, slug=?, slogan=?, description=?, whatsapp=?, phone=?, instagram_url=?, facebook_url=?, address=?, address_postal_code=?, address_street=?, address_number=?, address_complement=?, address_neighborhood=?, address_city=?, address_state=?, maps_url=?, timezone=?, special_message=?, theme=?, primary_color=?, public_site_url=?, seo_title=?, seo_description=?, updated_at=strftime('%Y-%m-%dT%H:%M:%fZ', 'now') WHERE id=1`).bind(
    ...
    input.mapsUrl, input.timezone, input.specialMessage, input.theme, input.primaryColor, input.publicSiteUrl, input.seoTitle, input.seoDescription,
  ).run()
  await invalidatePublicHtmlCache(c.req.url)
